...
__global__ void viterbi_kernel(int n_states, int n_possible_obs,
                               int n_actual_obs, double *trans_matrix,
                               double *emission_matrix, int *global_actual_obs,
                               double *global_probs, int *global_backpaths,
                               int current_t);

__host__ int *viterbi_cuda(int n_states, int n_possible_obs, int n_actual_obs,
                           int *actual_obs, double *init_probs,
                           double **transition_matrix,
                           double **emission_matrix) {

    int *backpaths = (int *) calloc(n_actual_obs * n_states, sizeof *backpaths);
    int *optimal_path = (int *) malloc(n_actual_obs * sizeof *optimal_path);
    ...
    cudaMalloc(&gpu_probs, n_states * n_actual_obs * sizeof *gpu_probs);
    ...
    cudaMemcpy(gpu_actual_obs, actual_obs,
               n_actual_obs * sizeof *gpu_actual_obs, cudaMemcpyHostToDevice);

    for (int i = 0; i < n_states; i++) {
        cudaMemcpy(gpu_trans + i * n_states, transition_matrix[i],
                   n_states * sizeof *gpu_trans, cudaMemcpyHostToDevice);
        cudaMemcpy(gpu_emission + i * n_possible_obs, emission_matrix[i],
    ...
    for (int i = 0; i < n_states; i++) {
        probs[i] = init_probs[i] + emission_matrix[i][actual_obs[0]];
    }
    cudaMemcpy(gpu_probs, probs, n_states * sizeof *gpu_probs,
               cudaMemcpyHostToDevice);
    ...
    for (int i = 1; i < n_actual_obs; ++i) {
        viterbi_kernel << < n_states, THREADS_PER_BLOCK, shared_mem_size >> > (
                n_states, n_possible_obs, n_actual_obs, gpu_trans, gpu_emission,
                        gpu_actual_obs, gpu_probs, gpu_backpaths, i);
...
__global__ void viterbi_kernel(int n_states, int n_possible_obs,
    ...
    double dev_emission_cell =
            emission_matrix[bidx * n_possible_obs + global_actual_obs[current_t]];
    ...
    for (int j = offset; j < n_states && j < offset + chunk_size; j++) {
        p_temp = global_probs[(current_t - 1) * n_states + j] +
                 dev_trans_probs[j] + dev_emission_cell;
        if (p_temp > p_max) {
            p_max = p_temp;
            i_max = j;
        }
    ...
    if (tidx < 16) {
        if (dev_max_probs[tidx] < dev_max_probs[tidx + 16]) {
        ...
        if (dev_max_probs[tidx] < dev_max_probs[tidx + 4]) {
            dev_max_probs[tidx] = dev_max_probs[tidx + 4];
            dev_max_indices[tidx] = dev_max_indices[tidx + 4];
        }
        if (dev_max_probs[tidx] < dev_max_probs[tidx + 2]) {
            dev_max_probs[tidx] = dev_max_probs[tidx + 2];
            dev_max_indices[tidx] = dev_max_indices[tidx + 2];
        }
        if (dev_max_probs[tidx] < dev_max_probs[tidx + 1]) {
            dev_max_probs[tidx] = dev_max_probs[tidx + 1];
...
__global__ void back_track_kernel(double *probs, int n_threads, int *out_idx) {
    ...
    if (tidx < 16) {
        if (max_probs[tidx] < max_probs[tidx + 16]) {
            max_probs[tidx] = max_probs[tidx + 16];
            max_idx[tidx] = max_idx[tidx + 16];
        }
        if (max_probs[tidx] < max_probs[tidx + 8]) {
            max_probs[tidx] = max_probs[tidx + 8];
            max_idx[tidx] = max_idx[tidx + 8];
        }
        if (max_probs[tidx] < max_probs[tidx + 4]) {
            max_probs[tidx] = max_probs[tidx + 4];
            max_idx[tidx] = max_idx[tidx + 4];
        }
        if (max_probs[tidx] < max_probs[tidx + 2]) {
            max_probs[tidx] = max_probs[tidx + 2];
            max_idx[tidx] = max_idx[tidx + 2];
        }
        if (max_probs[tidx] < max_probs[tidx + 1]) {
            max_probs[tidx] = max_probs[tidx + 1];
